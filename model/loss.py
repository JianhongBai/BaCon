...
class SemiConLoss(torch.nn.Module):
    def __init__(self, temperature=0.07, contrast_mode='all',
    ...
    def forward(self, features, soft_mask=None, ambiguous_mask=None):
        device = (torch.device('cuda')
                  if features.is_cuda
                  else torch.device('cpu'))
        ...
        # tile mask
        # mask = mask.repeat(anchor_count, contrast_count)
        if not len(logits) == len(soft_mask):
            soft_mask = soft_mask.repeat(anchor_count, contrast_count)
